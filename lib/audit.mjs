...
export function auditRepositoryState({
  ...
  if (action === "commit" && selectedSet.size) {
    ...
    const staleSelected = unique(selectedPaths)
      .filter((filePath) => !dirtySet.has(normalizePath(filePath)));
    if (staleSelected.length) {
      findings.push({
        code: "selected-paths-stale",
        severity: "blocked",
        message: "选中的文件已经不在当前变更列表中，请刷新仓库状态后重新选择。",
        paths: staleSelected
      });
    }
  }
  ...
  logger?.audit(action, verdict, findings);
...
function auditTitle(verdict, findings) {
  ...
  if (findings.some((finding) => finding.code === "selected-paths-stale")) return "选择已过期";
