...
export async function collectGitState(repoPath, options = {}) {
  ...
  const mergeInProgress = existsSync(path.join(repoRoot, ".git", "MERGE_HEAD"));
  const mergeMessage = mergeInProgress ? await readMergeMessage(repoRoot) : "";
  ...
  logger?.info("state", traceId, "collectGitState complete", {
    ...
    mergeInProgress: result.mergeInProgress,
...
export function summarizeGitState(state) {
  ...
  return {
    ...
    mergeInProgress: Boolean(state.mergeInProgress),
    mergeMessage: state.mergeMessage || "",
    blockers
  };
}

async function readMergeMessage(repoRoot) {
  try {
    return await readFile(path.join(repoRoot, ".git", "MERGE_MSG"), "utf8");
  } catch {
    return "";
  }
}
