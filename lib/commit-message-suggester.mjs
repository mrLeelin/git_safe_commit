...
export async function suggestCommitMessage(options = {}) {
  ...
  const prompt = buildPrompt(combinedDiff, normalizedPaths);
...
function buildPrompt(diff, paths = []) {
  const truncatedDiff = diff.length > MaxDiffChars ? `${diff.slice(0, MaxDiffChars)}\n\n[diff truncated]` : diff;
  const selectedFileList = formatSelectedFileList(paths);
  return [
    "你是严格的 Git 提交说明生成器。",
    "只分析这些已选择文件的 diff，不要推断未出现在 diff 里的仓库改动。",
    "多文件选择时必须覆盖所有已选择路径代表的变更范围；不能只根据第一个 diff 块生成说明。",
    "如果已选择文件很多，不要逐个机械罗列，但必须按目录、功能、资源、配置、测试或维护目的归组说明，避免遗漏后续文件。",
    "先在内部完成分析：识别用户意图、行为变化、触发场景、安全边界、测试/配置/样式/资源影响，以及是否只是重构或维护。",
    ...
    "按提交目的归并，不要按文件逐条罗列；多个文件服务同一目的时合并为一组描述。",
    "挑重点摘要：优先写主要目的、覆盖范围、用户/流程可感知影响，以及 diff 能证明的关键验证；不要展开次要文件细节。",
    "优先说明用户可感知的行为变化，其次说明安全边界、数据流或测试覆盖变化；避免只复述函数名、变量名和机械实现细节。",
    "如果同一分类下有必要补充细节，使用缩进后的 -- 续行说明关键影响、保护条件或测试范围。",
    "无法从 diff 判断目的时，使用保守描述，不要编造业务背景。",
    "详细说明模式不允许只输出单行；在 diff 支持时必须输出 2 到 4 行。",
    "如果第一行已经概括完主要目的，至少补充一条续行，说明影响范围、触发原因、保护条件、验证依据，或明确 diff 中没有可证明的运行时影响。",
    "维护或删除类变更也要补充续行，例如说明删除对象、为什么不影响运行路径、或本次只清理临时/生成文件；不要超过 4 行。",
    "",
    selectedFileList,
    "",
    truncatedDiff
...
function formatSelectedFileList(paths = []) {
  const normalized = Array.isArray(paths) ? paths.map(String).filter(Boolean) : [];
  if (!normalized.length) return "已选择文件列表 (0 个)：";
  return [
    `已选择文件列表 (${normalized.length} 个)：`,
