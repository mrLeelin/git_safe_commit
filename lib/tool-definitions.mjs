export const GitSafeCommitTools = [
  ...
  },
  {
    ...
    function: {
      name: "git_push",
      description: "Push current branch after the browser captured confirmation. This tool first checks remote freshness; if someone else already pushed, it blocks and tells the UI to sync remote before retrying. Never force push.",
      parameters: {
        type: "object",
        properties: {
          confirmed: { type: "boolean", description: "True only when the browser UI captured explicit push confirmation." }
        },
        required: []
      }
    }
...
- For ai-commit actions, read payload.paths and payload.message from the user JSON, stage only those exact paths with git_add, commit with that exact message, then final_verify.
- For commit operations, never stage paths that were not explicitly provided by the UI payload.
- For push operations, respect requireConfirmBeforePush; if confirmation is missing, escalate to the human UI.
- For ai-push actions, pass payload.confirmed to git_push.
- If git_push reports that the remote already has new commits, stop and tell the user to run 同步远端 before pushing again.
