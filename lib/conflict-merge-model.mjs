...
function chooseAutoKeyColumn(tables, columnCount) {
  const headerColumn = detectHeaderKeyColumn(tables, columnCount);
  if (headerColumn >= 0 && isUsableKeyColumn(tables, headerColumn) && !hasBlankKeyRows(tables, headerColumn)) return headerColumn;

  const heuristicColumn = detectHeuristicKeyColumn(tables, columnCount);
  if (heuristicColumn < 0) return -1;
  if (hasBlankKeyRows(tables, heuristicColumn)) return -1;
  return tableShapeSuggestsRowIdentityChanged(tables, heuristicColumn) ? heuristicColumn : -1;
}

function hasBlankKeyRows(tables, column) {
  return tables.some((table) => table.slice(1).some((row) => !normalizeKey(row?.[column])));
}
...
function keyRows(table, keyColumn) {
  ...
  for (let index = 0; index < table.length; index++) {
    ...
    const rawKey = normalizeKey(row?.[keyColumn]) || `__row:${index}`;
    const occurrence = (occurrences.get(rawKey) || 0) + 1;
    occurrences.set(rawKey, occurrence);
    const key = occurrence === 1 ? rawKey : `${rawKey}#${occurrence}`;
    keys.push(key);
