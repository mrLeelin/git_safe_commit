...
import { buildTableMerge } from "./conflict-merge-model.mjs";
...
const TextConflictExtensions = new Set([
  ...
  ".cginc", ".compute", ".uss", ".uxml", ".unity", ".prefab", ".asset", ".meta", ".mat", ".anim",
  ".controller", ".overridecontroller", ".playable", ".mask", ".physicmaterial", ".physicsmaterial2d"
]);
...
export async function loadTableConflict({ repoPath, filePath, runGit = runGitDefault } = {}) {
  ...
  if (SpreadsheetConflictExtensions.has(ext)) {
    return loadSpreadsheetTableConflict({ repoPath, relative, runGit });
  }
...
export async function writeTableCandidate({ repoPath, filePath, content = "", source = "table", cellChoices = [], sheetName = "", sheets = [], runGit = runGitDefault } = {}) {
  const relative = validateConflictPath(repoPath, filePath);
  assertTableConflict(relative);
  ...
  const choices = {
    ...
    sheets: Array.isArray(sheets) ? sheets.map((sheet) => ({ name: String(sheet.name || ""), contentLength: String(sheet.content || "").length })) : [],
    cellChoices: Array.isArray(cellChoices) ? cellChoices : [],
    contentLength: Array.isArray(sheets) && sheets.length
      ? sheets.reduce((total, sheet) => total + String(sheet.content || "").length, 0)
  ...
  if (SpreadsheetConflictExtensions.has(ext.toLowerCase())) {
    ...
    await writeFile(outputPath, await tableTextToWorkbookBuffer(String(content), { templateBuffer: ours.stdout, sheetName: choices.sheetName, sheets }));
  } else {
    await writeFile(outputPath, String(content), "utf8");
  }
  await writeFile(choicePath, JSON.stringify(choices, null, 2), "utf8");
...
function assertTextConflict(filePath) {
  ...
  if (!TextConflictExtensions.has(ext) && !TextConflictExtensions.has(baseName)) {
...
async function loadSpreadsheetTableConflict({ repoPath, relative, runGit }) {
  const [baseRaw, oursRaw, theirsRaw] = await Promise.all([
    readGitStageRaw(repoPath, relative, 1, runGit),
    readGitStageRaw(repoPath, relative, 2, runGit),
    readGitStageRaw(repoPath, relative, 3, runGit)
  ]);
  const [baseSheets, oursSheets, theirsSheets] = await Promise.all([
    workbookBufferToSheetCsvMap(baseRaw.ok ? baseRaw.stdout : Buffer.alloc(0)),
    workbookBufferToSheetCsvMap(oursRaw.ok ? oursRaw.stdout : Buffer.alloc(0)),
    workbookBufferToSheetCsvMap(theirsRaw.ok ? theirsRaw.stdout : Buffer.alloc(0))
  ]);
  const sheetNames = unionSheetNames(baseSheets, oursSheets, theirsSheets);
  const sheets = sheetNames.map((name) => {
    const base = spreadsheetStageInfo(baseRaw, 1, baseSheets, name);
    const ours = spreadsheetStageInfo(oursRaw, 2, oursSheets, name);
    const theirs = spreadsheetStageInfo(theirsRaw, 3, theirsSheets, name);
    return {
      name,
      base,
      ours,
  ...
  const activeSheet = sheets.find((sheet) => sheet.merge.conflictCount > 0)
    || sheets.find((sheet) => sheet.merge.autoCount > 0)
    || sheets[0]
    || {
      name: "Sheet1",
      base: spreadsheetStageInfo(baseRaw, 1, baseSheets, "Sheet1"),
      ours: spreadsheetStageInfo(oursRaw, 2, oursSheets, "Sheet1"),
      theirs: spreadsheetStageInfo(theirsRaw, 3, theirsSheets, "Sheet1"),
      merge: buildTableMerge("", "", "", { delimiter: "," })
    };
  ...
  return {
    ...
    tableConflict: {
      ...
      ours: activeSheet.ours,
      theirs: activeSheet.theirs,
      merge: activeSheet.merge
    }
  };
}
...
async function workbookBufferToSheetCsvMap(buffer) {
  const result = new Map();
  if (!buffer?.length) return result;
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(buffer);
  for (const sheet of workbook.worksheets) {
    result.set(sheet.name, worksheetToCsv(sheet));
  }
  return result;
}
...
function unionSheetNames(...sheetMaps) {
  const names = [];
  for (const sheetMap of sheetMaps) {
    for (const name of sheetMap.keys()) {
...
function spreadsheetStageInfo(rawResult, stage, sheetMap, sheetName) {
  return {
    stage,
    available: rawResult.ok && sheetMap.has(sheetName),
    content: sheetMap.get(sheetName) || "",
    byteLength: rawResult.ok ? rawResult.stdout.length : 0,
    error: rawResult.ok ? "" : (rawResult.stderr || rawResult.error || `stage ${stage} is unavailable`)
  };
}
...
async function tableTextToWorkbookBuffer(content, options = {}) {
  ...
  if (options.templateBuffer?.length) {
    ...
    if (Array.isArray(options.sheets) && options.sheets.length) {
      for (const sheetDraft of options.sheets) {
        const name = String(sheetDraft.name || "").trim();
        const target = (name ? workbook.getWorksheet(name) : null) || workbook.worksheets[0] || workbook.addWorksheet(name || "Sheet1");
        applyTableRowsToSheet(target, parseCsvTable(String(sheetDraft.content || "")));
      }
      return Buffer.from(await workbook.xlsx.writeBuffer());
    }
    sheet = options.sheetName ? workbook.getWorksheet(options.sheetName) : null;
    sheet = sheet || workbook.worksheets[0] || workbook.addWorksheet(options.sheetName || "Sheet1");
  } else {
    sheet = workbook.addWorksheet(options.sheetName || "Sheet1");
  }
...
function applyTableRowsToSheet(sheet, rows) {
  ...
  const extraRows = sheet.rowCount - rows.length;
  if (extraRows > 0) {
    sheet.spliceRows(rows.length + 1, extraRows);
  }
}
