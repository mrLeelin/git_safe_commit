...
export async function loadConfig(configPath = defaultConfigPath(), options = {}) {
  ...
  const raw = JSON.parse(stripByteOrderMark(await readFile(configPath, "utf8")));
...
export function normalizeConfig(raw) {
  ...
  return {
    ...
    repositories: normalizeRepositories(merged.repoPath, merged.repositories),
    server: {
...
function normalizeRepositories(activeRepoPath, repositories = []) {
  const active = path.resolve(String(activeRepoPath || ""));
  const seen = new Set();
  const result = [];
  for (const repo of [active, ...repositories]) {
    if (!repo) continue;
    const value = path.resolve(String(repo));
