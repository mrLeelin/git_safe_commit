...
export function createWorkflowRunner({ config, emit = () => {}, fetchImpl, runProcess: runProcessImpl } = {}) {
  ...
  async function syncRemote() {
    ...
    if (!fetchedSummary.cleanWorktree) {
      await setPhase("UserResolutionPending", "sync requires clean worktree");
      throw new Error("sync requires clean worktree; commit or discard local changes before syncing");
    }
    if (fetchedSummary.blockers.length) {
      await setPhase("UserResolutionPending", "sync blocked by repository state");
      throw new Error(`sync blocked: ${fetchedSummary.blockers.join("; ")}`);
    }
    ...
    return { ok: true, fetch, rebase, recovery, status, summary };
  }
  ...
  async function continueRebaseAndPush(payload = {}) {
    ...
    return { ok: true, continueRebase, push, status, summary };
  }
  ...
  async function pushDirect(payload = {}) {
    ...
    if (!beforeSummary.cleanWorktree) {
      ...
      throw new Error("push requires clean worktree; commit or resolve local changes before pushing");
    }
    if (config.workflow?.requireConfirmBeforePush && !payload.confirmed) {
      state.blockers = ["Push requires browser confirmation."];
      await setPhase("UserResolutionPending", "push confirmation required");
    ...
    return { ok: true, push, status, summary };
  }
