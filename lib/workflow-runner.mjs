...
import { currentTraceId, generateTraceId, getLogger, runWithTraceId } from "./logger.mjs";
...
export function resolveCreatedStashSha({ stashResult, previousSha = "", currentSha = "" } = {}) {
  if (!stashResult?.ok) return "";
  const output = `${stashResult.stdout || ""}\n${stashResult.stderr || ""}`;
  if (/no local changes to save/i.test(output)) return "";
  if (!currentSha || currentSha === previousSha) return "";
  return currentSha;
}

export function canProceedWithoutNewStash({ stashResult, previousSha = "", currentSha = "", status = "", untrackedFiles = "" } = {}) {
  if (resolveCreatedStashSha({ stashResult, previousSha, currentSha })) return false;
  const output = `${stashResult?.stdout || ""}\n${stashResult?.stderr || ""}`;
  if (!/no local changes to save/i.test(output)) return false;
  const entries = String(status).split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  return entries.length > 0
    && entries.every((line) => /^\?\? .+\/$/.test(line))
    && !String(untrackedFiles).trim();
}

export function canContinueRebase({ requiresConfirmation = false, confirmed = false } = {}) {
  return !requiresConfirmation || Boolean(confirmed);
}

const MutatingActions = new Set([
  "create-recovery", "fetch", "sync", "merge", "create-branch", "commit", "discard-selected",
  "restore-tool-stashes", "continue-rebase-and-push", "abort-rebase", "push",
  "ai-sync-and-push", "ai-sync", "ai-push", "ai-commit"
]);
...
export function createWorkflowRunner({ config, emit = () => {}, fetchImpl, runProcess: runProcessImpl } = {}) {
  ...
  async function run(action, payload = {}) {
    ...
    return runWithTraceId(traceId, async () => {
      ...
      if (MutatingActions.has(action)) {
        if (activeMutation) {
          logger?.warn("workflow", traceId, `操作被拒绝（已有操作在运行）: ${action}`);
      ...
      try {
        let result;
        if (action === "inspect") result = await inspect();
        else if (action === "create-recovery") result = await createRecoveryAction();
        else if (action === "fetch") result = await fetchRemote();
        else if (action === "sync") result = await syncRemote();
        else if (action === "merge") result = await mergeBranch(payload);
        else if (action === "create-branch") result = await createBranch(payload);
        else if (action === "commit") result = await commitSelectedFiles(payload);
        else if (action === "discard-selected") result = await discardSelectedFiles(payload);
        else if (action === "restore-tool-stashes") result = await restoreToolStashesAction(payload);
        else if (action === "continue-rebase-and-push") result = await continueRebaseAndPush(payload);
        else if (action === "abort-rebase") result = await abortRebase(payload);
        else if (action === "push") result = await pushDirect(payload);
        else if (action === "ai-sync-and-push") result = await syncAndPush(payload);
        else if (action === "ai-sync") result = await runAiSyncAction(payload);
        else if (action === "ai-push" || action === "ai-commit") result = await runAiAction(action, payload);
        else throw new Error(`unknown workflow action: ${action}`);

        const duration = Date.now() - startTime;
        logger?.info("workflow", traceId, `操作完成: ${action}`, { ok: result?.ok, duration });
        return result;
      } catch (error) {
        const duration = Date.now() - startTime;
        logger?.error("workflow", traceId, `操作失败: ${action}`, { duration, mutating: MutatingActions.has(action) }, error);
        throw error;
      } finally {
        if (MutatingActions.has(action)) activeMutation = false;
      }
    });
  }
  ...
  async function inspectSnapshot() {
    ...
    const logger = getLogger();
    logger?.info("audit", currentTraceId(), `审计结果: ${audit.verdict}`, {
  ...
  async function syncRemote() {
    ...
    const excelBlock = await excelPreflightBlockedResult({ status: fetchedStatus, summary: fetchedSummary, fetch });
    if (excelBlock) return excelBlock;
  ...
  async function mergeBranch(payload = {}) {
    ...
    if (!branch || !/^[A-Za-z0-9._\/-]+$/.test(branch) || branch.includes("..") || branch.includes("@{")) {
      throw new Error("merge requires a valid named source branch");
    }
    if (!payload.confirmed) {
      return { ok: false, blocked: true, reason: "merge confirmation required" };
    }

    const beforeStatus = await collectGitState(config.repoPath);
    const beforeSummary = summarizeGitState(beforeStatus);
    if (beforeSummary.rebaseInProgress || beforeSummary.unmergedCount) {
      throw new Error("merge blocked: repository is already resolving a rebase or conflict");
    }
    if (!beforeSummary.cleanWorktree) {
      throw new Error(`merge requires a clean worktree: ${(beforeSummary.blockers || []).join("; ")}`);
    }

    await setPhase("Merging", `merging ${branch}`);
    const merge = await runGit(config.repoPath, ["merge", "--no-ff", "--no-edit", "--", branch]);
    const status = await collectGitState(config.repoPath);
    const summary = summarizeGitState(status);
    state.blockers = summary.blockers;
    if (!merge.ok) {
      await setPhase("UserResolutionPending", "merge needs conflict resolution");
      return { ok: false, blocked: true, reason: merge.stderr || merge.stdout || merge.error || "git merge failed", merge, status, summary };
    }
    const recoveryCleanup = state.activeRecovery ? await cleanupRecovery(state.activeRecovery) : null;
    await setPhase("Idle", "merge complete");
    return {
      ok: true,
      message: `merge committed: ${branch} into ${beforeSummary.branch}`,
      mergeCommit: {
        sourceBranch: branch,
        targetBranch: beforeSummary.branch
      },
      merge,
      recoveryCleanup,
      status,
  ...
  async function createBranch(payload = {}) {
    const branch = typeof payload.branch === "string" ? payload.branch.trim() : "";
    if (!branch || !/^[A-Za-z0-9._\/-]+$/.test(branch) || branch.includes("..") || branch.includes("@{") || branch.startsWith("/") || branch.endsWith("/") || branch.includes("//")) {
      throw new Error("create branch requires a valid named branch");
    }
    if (!payload.confirmed) {
      return { ok: false, blocked: true, reason: "create branch confirmation required" };
    }

    const beforeStatus = await collectGitState(config.repoPath);
    const beforeSummary = summarizeGitState(beforeStatus);
    if (beforeSummary.rebaseInProgress || beforeSummary.unmergedCount) {
      throw new Error("create branch blocked: repository is already resolving a rebase or conflict");
    }
    if (!beforeSummary.cleanWorktree) {
      throw new Error(`create branch requires a clean worktree: ${(beforeSummary.blockers || []).join("; ")}`);
    }
    const branchExists = beforeStatus.branches.includes(branch);
    await setPhase("BranchCreating", branchExists ? `switching to branch ${branch}` : `creating branch ${branch}`);
    const checkout = await runGit(config.repoPath, branchExists ? ["checkout", branch] : ["checkout", "-b", branch]);
    const status = await collectGitState(config.repoPath);
    const summary = summarizeGitState(status);
    state.blockers = summary.blockers;
    if (!checkout.ok) {
      await setPhase("UserResolutionPending", branchExists ? "branch switch failed" : "branch creation failed");
      return { ok: false, blocked: true, reason: checkout.stderr || checkout.stdout || checkout.error || (branchExists ? "branch switch failed" : "branch creation failed"), checkout, status, summary };
    }
    const message = branchExists ? `switched to branch ${branch}` : `branch ${branch} created`;
    await setPhase("Idle", message);
    return { ok: true, message, created: !branchExists, switched: true, checkout, status, summary };
  }
  ...
  async function stashForSync(timestamp, recovery = null) {
    ...
    const previous = await runGit(config.repoPath, ["rev-parse", "--verify", "stash@{0}"]);
    const previousSha = previous.ok ? previous.stdout.trim() : "";
    ...
    const ref = "stash@{0}";
    const current = await runGit(config.repoPath, ["rev-parse", "--verify", ref]);
    const sha = resolveCreatedStashSha({
      stashResult: stash,
      previousSha,
      currentSha: current.ok ? current.stdout.trim() : ""
    });
    if (!sha) {
      const status = await runGit(config.repoPath, ["status", "--short", "--untracked-files=normal"]);
      const untrackedFiles = await runGit(config.repoPath, ["ls-files", "--others", "--exclude-standard", "-z"]);
      if (canProceedWithoutNewStash({
        stashResult: stash,
        previousSha,
        currentSha: current.ok ? current.stdout.trim() : "",
        status: status.ok ? status.stdout : "",
        untrackedFiles: untrackedFiles.ok ? untrackedFiles.stdout : "unknown"
      })) {
        await setPhase("Stashing", "only empty untracked directories remain; no stash needed");
        return null;
      }
      await setPhase("UserResolutionPending", "temporary stash did not create a new entry");
      throw new Error("temporary stash did not create a new entry; refusing to restore an existing stash");
    }
    const syncStash = { message, ref, sha, stash, timestamp };
  ...
  async function commitSelectedFiles(payload = {}) {
    ...
    const { status: beforeStatus, summary: beforeSummary } = await auditBeforeCommit(paths);
    ...
    const add = await stageSelectedPaths(paths, beforeStatus);
    if (!add.ok) {
      await setPhase("UserResolutionPending", "git add failed");
      throw new Error(add.stderr || add.stdout || add.error || "git add failed");
    }
    const defaultMergeMessage = beforeSummary.mergeInProgress ? String(beforeStatus.mergeMessage || "").trim() : "";
    const commit = await runGit(
      config.repoPath,
      defaultMergeMessage && message === defaultMergeMessage
        ? ["commit", "--no-edit"]
        : ["commit", "-m", message]
    );
    if (!commit.ok) {
  ...
  async function continueRebaseAndPush(payload = {}) {
    ...
    if (!canContinueRebase({
      requiresConfirmation: config.workflow?.requireConfirmBeforePush,
    ...
    if (!push.ok) {
      if (isRemoteAdvancedPushFailure(push)) {
        ...
        const recoveryCleanup = state.activeRecovery ? await cleanupRecovery(state.activeRecovery) : null;
        return { ...blocked, continueRebase, push, recoveryCleanup, syncStash: syncStash ? { ...syncStash, ...stashRestore } : null };
      }
  ...
  function createToolHandlers({ action = "", payload = {} } = {}) {
    return {
      ...
      git_add: async ({ paths } = {}) => {
        ...
        return stageSelectedPaths(addPaths.map(String));
      },
      git_commit: async ({ message } = {}) => {
        ...
        if (action === "ai-commit") {
          await auditBeforeCommit(payload.paths);
        }
  ...
  async function auditBeforeCommit(paths = []) {
    const selectedPaths = Array.isArray(paths) ? paths.map(String) : [];
    const status = await collectGitState(config.repoPath);
    const summary = summarizeGitState(status);
    const activeSyncStash = await getActiveSyncStash();
    const toolStashes = await listToolStashes();
    ...
    if (audit.verdict === "blocked") {
      state.blockers = audit.findings
        .filter((finding) => finding.severity === "blocked")
        .map((finding) => finding.message);
      await setPhase("UserResolutionPending", "commit blocked by audit");
      throw auditError(blockedAuditMessage(audit), audit);
    }
    state.blockers = summary.blockers;
    return { status, summary, audit, toolStashes };
  }

  async function selectedIgnoredPaths(paths = []) {
    const selectedPaths = Array.isArray(paths) ? paths.map(String).filter(Boolean) : [];
    if (!selectedPaths.length) return [];
    const result = await runGit(config.repoPath, ["check-ignore", "--", ...selectedPaths]);
    if (result.code === 1 || !result.stdout.trim()) return [];
    if (!result.ok) {
      throw new Error(result.stderr || result.stdout || result.error || "git check-ignore failed");
    }
    const ignored = new Set(result.stdout.split(/\r?\n/).filter(Boolean));
    return selectedPaths.filter((filePath) => ignored.has(filePath));
  }

  async function stageSelectedPaths(paths = [], status = null) {
    const selectedPaths = Array.isArray(paths) ? paths.map(String).filter(Boolean) : [];
    const currentStatus = status || await collectGitState(config.repoPath);
    const stagedOnlyPaths = selectedStagedOnlyPaths(selectedPaths, currentStatus);
    const ignoredPaths = await selectedIgnoredPaths(selectedPaths);
    const addPaths = selectedPaths.filter((filePath) => {
      return !ignoredPaths.includes(filePath) && !stagedOnlyPaths.includes(filePath);
    });
    const add = addPaths.length
      ? await runGit(config.repoPath, ["add", "--", ...addPaths])
      : { ok: true, stdout: "", stderr: "", skippedIgnoredPaths: ignoredPaths };
    add.skippedIgnoredPaths = ignoredPaths;
    add.skippedStagedOnlyPaths = stagedOnlyPaths;
    return add;
  }
  ...
  async function setPhase(phase, note = "") {
    ...
    state.note = note;
    const logger = getLogger();
    logger?.debug("phase", currentTraceId(), `${phase} → "${note}"`);
  ...
  async function guardPushPreflight() {
    ...
    const summary = summarizeGitState(status);
    state.blockers = summary.blockers;
    const excelBlock = await excelPreflightBlockedResult({ status, summary, fetch });
    if (excelBlock) return excelBlock;
    if (summary.behind > 0) {
  ...
  async function excelPreflightBlockedResult({ status, summary, fetch = null } = {}) {
    const excelBlockers = (summary?.blockers || []).filter((blocker) => /Excel|\.xlsx|\.xlsm|\.xlsb|\.xls/i.test(blocker));
    if (!excelBlockers.length) return null;
    state.blockers = excelBlockers;
    await setPhase("UserResolutionPending", "close Excel workbook before rebase or push");
    const files = (status?.excel?.files || [])
      .filter((file) => file.lockedExclusive || file.openInExcel)
      .map((file) => file.path);
    const message = [
      "检测到即将 rebase/推送前会被 Git 覆盖或参与冲突的 Excel 表格仍在打开或被占用。",
      "请先关闭这些表格，然后再继续 rebase 或重新推送：",
      ...(files.length ? files : excelBlockers)
    ].join("\n");
    return {
      ok: false,
      blocked: true,
      reason: "excel workbook must be closed before rebase",
      message,
      error: message,
      recommendedAction: "close-excel-and-retry",
      blockers: excelBlockers,
      fetch,
      status,
      summary
    };
...
function blockedAuditMessage(audit = {}) {
  const blocked = (audit.findings || []).find((finding) => finding.severity === "blocked");
  if (!blocked) return "commit blocked by audit";
  const paths = Array.isArray(blocked.paths) && blocked.paths.length
    ? ` Paths: ${blocked.paths.join(", ")}`
    : "";
  return `${blocked.message || "commit blocked by audit"}${paths}`;
}
...
function selectedStagedOnlyPaths(paths = [], status = {}) {
  const staged = new Set((status.staged || []).map((item) => normalizePath(item.path)));
  const needsAdd = new Set([
    ...(status.unstaged || []).map((item) => item.path),
    ...(status.untracked || []),
    ...(status.unmerged || [])
  ...
  return paths.filter((filePath) => {
    ...
    return staged.has(normalized) && !needsAdd.has(normalized);
  });
}
