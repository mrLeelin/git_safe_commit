...
import { getLogger, generateTraceId } from "./logger.mjs";
...
const MutatingActions = new Set([
  "create-recovery", "fetch", "sync", "merge", "commit", "discard-selected",
...
export function createWorkflowRunner({ config, emit = () => {}, fetchImpl, runProcess: runProcessImpl } = {}) {
  ...
  async function inspectSnapshot() {
    ...
    const logger = getLogger();
    logger?.info("audit", state._traceId || "", `审计结果: ${audit.verdict}`, {
  ...
  async function mergeBranch(payload = {}) {
    ...
    return { ok: true, message: "merge complete", merge, recoveryCleanup, status, summary };
  }
  ...
  async function setPhase(phase, note = "") {
    ...
    state.note = note;
    const logger = getLogger();
    logger?.debug("phase", state._traceId || "", `${phase} → "${note}"`);
