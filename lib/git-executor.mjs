...
export async function runGit(repoPath, args, options = {}) {
  ...
  const commandStr = `git ${gitArgs.join(" ")}`;
  logger?.debug("git", traceId || "", commandStr, { cwd });
  ...
  if (result.stderr) extra.stderrPreview = result.stderr.slice(0, 500);
  logger?.debug("git", traceId || "", `→ exit:${result.code}`, extra);

  if (!result.ok) {
    logger?.warn("git", traceId || "", `git command failed: ${commandStr}`, {
      code: result.code,
